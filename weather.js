...
let displayResponse = (result) => {
    ...
    dayName.forEach((dName,index)=>{
        let date=new Date();
        dName.innerText=days[date.getDay(result.list[index])];
    });
    let min_max=document.querySelectorAll('.bottom-container div .min-max-temp');
    min_max.forEach((minMax,index)=>{
        minMax.innerText=`${Math.round(result.list[index].main.temp_min)}°C / ${Math.round(result.list[index].main.temp_max)}°C`
    });
